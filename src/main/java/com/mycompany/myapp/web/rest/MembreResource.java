...
import com.mycompany.myapp.service.MembreQueryService;
...
public class MembreResource {
    ...
    private final MembreQueryService membreQueryService;

    public MembreResource(MembreService membreService, MembreRepository membreRepository, MembreQueryService membreQueryService) {
        this.membreService = membreService;
        this.membreRepository = membreRepository;
        this.membreQueryService = membreQueryService;
    }
     ...
     */
    @GetMapping("/membres")
    public ResponseEntity<List<Membre>> getAllMembres(MembreCriteria criteria, Pageable pageable) {
        log.debug("REST request to get Membres by criteria: {}", criteria);
        Page<Membre> page = membreQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /membres/count} : count all the membres.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/membres/count")
    public ResponseEntity<Long> countMembres(MembreCriteria criteria) {
        log.debug("REST request to count Membres by criteria: {}", criteria);
        return ResponseEntity.ok().body(membreQueryService.countByCriteria(criteria));
    }
